...
import { getGroupById, createPost, joinGroup, leaveGroup} from "../services/socialService"
...
function GroupDetailPage() {
  ...
  const isMember = group?.Users?.some((m) => m.id_utilizator === currentUser?.id)
  
  const handleLeaveGroup = async () => {
    setError("")
    setSuccess("")
    try {
      await leaveGroup(id_grup)
      setSuccess("Ai părăsit grupul.")
      fetchGroupData()
    } catch (err) {
      console.error("Error leaving group:", err)
      setError(err.response?.data?.error || "A apărut o eroare la părăsirea grupului. Vă rugăm încercați din nou.")
    }
  }
  if (loading) {
